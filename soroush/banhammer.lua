...
local function kick_ban_res(extra, success, result)
	  ...
	  if success == 0 then
		return send_large_msg(receiver, "یوزرنیم کاربر مورد نظر یافت نشد!")
	  end
      ...
      local get_cmd = extra.get_cmd
       if get_cmd == "kick" then
         if member_id == from_id then
            send_large_msg(receiver, "شما نمی توانید خودتان را اخراج کنید!")
			return
         end
      ...
      elseif get_cmd == 'banall' then
        send_large_msg(receiver, 'کاربر @'..member..' ['..member_id..'] گلوبال بن شد (Globally Banned)')
		banall_user(member_id)
      elseif get_cmd == 'unbanall' then
        send_large_msg(receiver, 'کاربر @'..member..' ['..member_id..'] گلوبال آن بن شد (Globally unbanned)')
...
local support_id = msg.from.id
 if matches[1]:lower() == 'id' and msg.to.type == "chat" or msg.to.type == "user" then
    if msg.to.type == "user" then
      return "آی دی (ID) ربات: "..msg.to.id.. "\n\nآی دی (ID) شما:"..msg.from.id
    end
  ...
  if matches[1]:lower() == 'ban' then-- /ban
    ...
    elseif string.match(matches[2], '^%d+$') then
        ...
        if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
          	return "شما نمیتوانید ادمین ربات/ صاحب گروه/مدیر را بن کنید!"
        end
		...
		send_large_msg(receiver, 'کاربر ['..matches[2]..']بن شد (Banned)')
      else
		local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'ban',
  ...
  if matches[1]:lower() == 'unban' then -- /unban
    if type(msg.reply_id)~="nil" and is_momod(msg) then
      ...
      if string.match(targetuser, '^%d+$') then
        	...
        	return 'کاربر '..user_id..' آن بن شد (Unbanned)'
      else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'unban',
...
if matches[1]:lower() == 'kick' then
	...
	elseif string.match(matches[2], '^%d+$') then
		...
		if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
			return "شما نمیتوانید ادمین ربات/ صاحب گروه/مدیر را اخراج کنید!"
		end
		if tonumber(matches[2]) == tonumber(msg.from.id) then
			return "شما نمی توانید خودتان را اخراج کنید!"
		end
    local user_id = matches[2]
    local chat_id = msg.to.id
		local print_name = user_print_name(msg.from):gsub("‮", "")
      ...
      if string.match(targetuser, '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	banall_user(targetuser)
       		return 'کاربر ['..user_id..' ] گلوبال بن شد (Globally Banned)'
     else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'banall',
